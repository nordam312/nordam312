[README.md](https://github.com/user-attachments/files/28184550/README.1.md)
# Hi, I'm Muhammednur Damlahi

Full-Stack Software Engineer · Istanbul, Türkiye

I build production-grade SaaS products, AI-integrated systems, and developer tooling — end-to-end, from database schema to deployed infrastructure.

Open to remote opportunities with SaaS / B2B product companies.

---

## Featured Projects

### SnapCal — AI Nutrition Coach
**Live:** [snapcalcoach.com](https://snapcalcoach.com) · [Telegram Bot](https://t.me/SnapCalCoachBot)

Production SaaS for AI-powered meal logging via photo, voice, and text.
TypeScript · Node.js · Next.js 16 · Gemini Vision · GPT-4o-mini · Whisper · PostgreSQL · Redis · Docker · GitHub Actions · Paddle

### Syrian Community Platform — Istanbul Aydın University
**Live:** [https://iausyriancommunity.com/](https://iausyriancommunity.com/)

Full-stack platform serving 200+ students — networking, events, and the university's first digital council elections.
React · TypeScript · Laravel · MySQL · Tailwind CSS

---

## Technical Skills

| Area | Stack |
|---|---|
| **Languages** | TypeScript (strict), JavaScript (ES2023+), SQL, PHP |
| **Backend** | Node.js, Express, Telegraf, Prisma ORM, Zod, Laravel |
| **Frontend** | React, Next.js, Tailwind CSS v4 |
| **AI & LLM** | Gemini Vision, GPT-4o-mini, Whisper, prompt engineering |
| **Databases** | PostgreSQL, MySQL, Redis, PgBouncer |
| **DevOps** | Docker, GitHub Actions CI/CD, Caddy, Let's Encrypt, Linux |
| **Cloud** | Hetzner, Vercel, Neon, GHCR, SSH deployment |
| **Payments** | Paddle (HMAC webhooks, subscription lifecycle) |
| **Testing** | Unit, Integration (real DB), E2E (API mocking via nock) |

---

## Contact

📧 [nordam312@gmail.com](mailto:nordam312@gmail.com) · 💼 [LinkedIn](https://www.linkedin.com/in/mohammeddamlakhi/)
